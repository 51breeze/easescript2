...
class CallExpression extends Expression{
    ...
    check(){
        ...
        if( length > 0 ){
            ...
            const getParamType=( param ,arg)=>{
                param.on("FETCH_GENERIC_TYPE", (event)=>{
                    event.type = arg.type();
            ...
            const args = this.arguments.map( (item,index)=>{
                if( description.params[index] && description.params[index] !== rest ){
                    const acceptType =  event.type && event.target===description ? event.type : getParamType(description.params[index], item);

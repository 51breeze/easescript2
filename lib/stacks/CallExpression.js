        ...
        if( length > 0 ){
            ...
            const args = this.arguments.map( (item,index)=>{
                if( description.params[index] && description.params[index] !== rest ){
                    const acceptType =  event.type && event.target===description ? event.type : description.params[index].type();
                    if( acceptType && !acceptType.check(item, acceptType.isTupleType || acceptType.isGenericType ) ){

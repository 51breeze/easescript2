...
class CallExpression extends Expression{
    ...
    reference(){
        ...
        if( description.isFunctionExpression || description.isMethodDefinition){
            return description.scope.returnItems[0].reference();
        }
        if(description.isModule && description.methodConstructor){
            return description.methodConstructor.scope.returnItems[0].reference();
        }
        return null;
    }
    ...
    check(){
        ...
        if( this.genericity ){
            ...
            declareGenerics.forEach( (item,index)=>{
                ...
                if(  this.genericity[index].isObjectExpression )
                console.log( this.genericity[index].node.properties[0] )
        ...
        if( length > 0 ){
            ...
            const args = this.arguments.map( (item,index)=>{
                if( description.params[index] && description.params[index] !== rest ){
                    ...
                    if( acceptType && !acceptType.check(item, acceptType.isTupleType,isGeneric ) ){
                        let valueType = item.type().toString();
                        if( acceptType.isTupleType ){
                            const ref = item.reference();
                            if( ref.isArrayExpression ){

        ...
        if( length > 0 ){
            ...
            const args = this.arguments.map( (item,index)=>{
                ...
                if( description.params[index] && description.params[index] !== rest ){
                    const acceptType =  event.tupleType && event.target===description ? new UnionType(event.tupleType.elements) : description.params[index].type();
                    if( acceptType && !acceptType.check( item ) ){

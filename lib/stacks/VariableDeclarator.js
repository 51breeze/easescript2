...
class VariableDeclarator extends Declarator {
    ...
    { 
        ...
        if( Utils.isStackByName(this.id,"Identifier") ){
            this.assignValue = this.init;
            scope.define( this.id.value(), this );
        }else if( Utils.isStackByName(this.id,"ObjectPattern") || Utils.isStackByName(this.id,"ArrayPattern") ){
            this.isPattern = true;
    ...
    emit(syntax){
        return syntax.builder(this,"VariableDeclarator");
    }
}

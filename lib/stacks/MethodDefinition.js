...
class MethodDefinition extends Stack{
    ...
    constructor(compilation,node,scope,parentNode,parentStack)
    {
        ...
        this.kind = node.kind;
        this.genericType = Utils.createStack(compilation,node.key.genericType,scope,node,this);
    ...
    parser(syntax){
        ...
        if( this.override ){
            if( !parent || !parent.getMember( this.key.value() ) ){
                this.throwError(`"${this.key.value()}" does not exists in parent class. remove the "override" modifier if is not overwrite.`)
            }
        }else if( parent ){
            const parentMethod = parent.getMember( this.key.value() );
            if( parentMethod && parentMethod.modifier.value() !=="private" ){
                this.throwError(`"${this.key.value()}" already exists in parent class. use the "override" modifier if need overwrite`)
            }
        }

...
class TypeDefinition extends Stack {
    ...
    type(){
        if( this.typeElements ){
           const restElement = this.typeElements[ this.typeElements.length-1 ].restElement;
           return this.tupleType || (this.tupleType = new TupleType( this.typeElements.map( item=>item.type() ) , restElement ));
        } 

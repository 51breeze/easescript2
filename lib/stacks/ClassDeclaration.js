...
class ClassDeclaration extends Stack{
    ...
    implementCheck(module, stack ){
        ...
        const check = (left,right)=>{
            ...
            if( !right ){
                ...
                if( !Utils.isStackByName(right,"MethodDefinition") ){
                    right.throwError(`the "${left.value()}" member method inconformity at the interface of ${module.getName()} in class of ${this.compilation.module.getName()}`);
                }
                if( left.params.length > right.params.length ){
                    right.key.throwError(`the "${left.value()}" member method params missing at the interface of ${module.getName()} in class of ${this.compilation.module.getName()}`);
                }
                ...
                if( !result ){
                    right.key.throwError(`the "${left.value()}" member method params type not matched. at the interface of ${module.getName()} in class of ${this.compilation.module.getName()}`);
                }
            }
        }
        for(var name in members){
           ...
           if( left.isAccessor ){
               check(left.get, right && right.get);
               check(left.set, right && right.set);
           }else{
               check(left, right);
           }
        }

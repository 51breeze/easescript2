const Stack = require("../Stack");
const Utils = require("../Utils");
const ClassScope = require("../scope/ClassScope");
class ClassDeclaration extends Stack{

    constructor(compilation,node,scope,parentNode,parentStack){
        scope = new ClassScope(scope);
        super(compilation,node,scope,parentNode,parentStack);
        this.metatypes = [];
        this.annotations = [];
        this.id          = Utils.createStack(compilation,node.id,scope,node,this);
        this.inherit     = Utils.createStack(compilation,node.superClass,scope,node,this);
        this.static      = Utils.createStack(compilation,node.static,scope,node,this);
        this.implements  = (node.implements || []).map((item)=>Utils.createStack(compilation,item,scope,node,this));
        this.modifier = Utils.createStack(compilation,node.modifier,scope,node,this);
        const MetatypeDeclaration = Utils.getStackByName('MetatypeDeclaration');
        const AnnotationDeclaration = Utils.getStackByName('AnnotationDeclaration');
        const metatypes = [];
        const annotations = [];
        this.body = (node.body.body || []).map( item=>{
            const stack = Utils.createStack( compilation, item, scope, node ,this);
            if( stack instanceof MetatypeDeclaration ){
                metatypes.push( stack );
            }else if( stack instanceof AnnotationDeclaration ){
                annotations.push( stack );
            }else{
                stack.metatypes   = metatypes.splice(0,metatypes.length);
                stack.annotations = metatypes.splice(0,annotations.length);
                return stack;
            }
        }).filter( item=>!!item );
        compilation.module.id = this.id.value();
        compilation.module.isClass = true;
        compilation.module.implements = this.implements.map( (stack)=>{
            const module = compilation.getType( stack.value() );
            if( module ){
                if( !module.isInterface ){
                    stack.throwError(`"${stack.value()}" is not interface`);
                }
                compilation.module.addDepend( module.getName(), module );
            }else {
                stack.throwError(`"${stack.value()}" is not exists`);
            }
            this.implementCheck( module );
            return module;
        });
        if( !compilation.module.checkClassName( compilation.module.id ) ){
            this.id.throwError(`"${this.id.value()}" class identifier is not consistent with the file name.`);
        }
        if( this.inherit ){
            const module = compilation.getType( this.inherit.value() );
            compilation.module.extends = module;
            if( module ){
                compilation.module.addDepend( module.getName(), module );
            }else{
                this.inherit.throwError(`"${ this.inherit.value()}" is not exists`)
            }
        }
        compilation.module.publish();
    }

    implementCheck(interfaceModule){
        const check = (left,right)=>{
            if( !left )return;
            if( !right ){
                left.throwError(`the "${left.value()}" method is not implemented the method in the "${interfaceModule.getName()}" interface.`);
            }else if( !Utils.checkTypeForBoth(left.type(),right.type()) ){
                right.throwError(`the "${right.value()}" member method return type does not matched with the method return type in the "${interfaceModule.getName()}" interface.`);
            }else if( Utils.isStackByName(left,"MethodDefinition") ){
                if( !Utils.isStackByName(right,"MethodDefinition") ){
                    right.throwError(`the "${left.value()}" member method inconformity with the method in the "${interfaceModule.getName()}" interface.`);
                }
                if( left.params.length > right.params.length ){
                    right.throwError(`the "${left.value()}" member method params missing with the method params in the "${interfaceModule.getName()}" interface.`);
                }
                const result = left.params.every( (item,index)=>{
                    return right.params[index] ? Utils.checkTypeForBoth(item.type(),right.params[index].type() ) : false;
                });
                if( !result ){
                    right.throwError(`the "${left.value()}" member method params type not matched with the method params in the "${interfaceModule.getName()}" interface.`);
                }
                const lGens = left.genericity ? left.genericity.length : 0;
                const rGens = right.genericity ? right.genericity.length : 0;
                if( rGens > 0 && lGens !== rGens ){
                    right.throwError(`the "${left.value()}" member method generics is not consistent with the method generics in the "${interfaceModule.getName()}" interface.`);
                }
                if( rGens > 0 ){
                    const result= right.genericity.every( (right,index)=>{
                        const left = left.genericity[index];
                        if( !left || right.extends.length !== left.extends.length )return false;
                        if( right.extends.length > 0 ){
                            return right.extends.every((type)=>left.extends.some(left=>type.constraint(left)));
                        }
                        return true;
                    });
                    if( !result ){
                        right.throwError(`the "${left.value()}" member method generics is not consistent with the method generics in the "${interfaceModule.getName()}" interface.`);
                    }
                }
            }
            if( right.modifier && right.modifier.value() !=="public" ){
                right.throwError(`the "${right.value()}" member method modifier is not consistent with the method modifier in the "${interfaceModule.getName()}" interface.`);
            }
        }
        const members = interfaceModule.members || {};
        for(var name in members){
           const left = members[name];
           if( left.isAccessor ){
               check(left.get,this.compilation.module.getMember(name,"get"));
               check(left.set,this.compilation.module.getMember(name,"set"));
           }else{
               check(left,this.compilation.module.getMember(name));
           }
        }
        interfaceModule.extends.forEach( item=>{
            this.implementCheck(item);
        });
        interfaceModule.implements.forEach( item=>{
            this.implementCheck(item);
        });
    }

    parser(syntax){
        this.body.forEach(item =>{
            item.parser(syntax);
        });
    }

    emit( syntax ){
        const body = this.body.map( item=>item.emit(syntax) ).filter( item=>!!item );
        return body.join("\r\n");
    }
}

module.exports = ClassDeclaration;
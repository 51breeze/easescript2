...
class IfStatement extends Stack{
    ...
    emit(syntax){
        ...
        if(this.hasAwait){
            const stack = this.getParentStackByName("BlockStatement");
            if( !alternate ){
                return `if(!${condition})return [3,${stack.awaitLabelIndex}];\r\n${consequent}\r\ncase ${stack.awaitLabelIndex}:\r\n`;
            }else{
                return `if(!${condition})return [3,${this.alternate.awaitLabelIndex}];\r\n${consequent}\r\ncase ${this.alternate.awaitLabelIndex}:\r\n${alternate}`; 
            }
        }
        if( alternate ){

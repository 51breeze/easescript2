...
class BinaryExpression extends Expression{
     ...
     check(){
          ...
          if( operator ==="instanceof" || operator ==="is" ){
               ...
               if( left ){
                    const check = (stack)=>{
                         return stack.isLiteral || stack.isTypeDefinition || this.compilation.getTypeWhenExist(stack.value());
                    };
               ...
               if( right ){
                    if(Array.isArray(right) && right.some( item=>!( !item.isLiteral && this.compilation.getType(item.value()) ) ) ){
                         this.throwError(`"${operator}" operator right-hand expression is not class type`);
                    }else if( right instanceof Stack && !(!right.isLiteral && this.compilation.getType( right.value() ) ) ){
                         this.throwError(`"${operator}" operator right-hand expression is not class type`);
                    }
               }
          }

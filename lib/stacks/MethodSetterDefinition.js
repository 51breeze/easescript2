...
class MethodSetterDefinition extends MethodDefinition{
    ...
    parser(syntax){
        super.parser(syntax);
        if( this.value.params.length != 1 ){
            this.throwError(`"${this.key.value()}" setter must have one param`);
        }
        const param = this.value.params[0];
    ...
    assignment( value ){
        ...
        if( acceptType && !acceptType.check( value ) ){
            this.throwError(`"${this.key.raw()}" not match of type ${acceptType.toString()} to assignment type ${value.type().toString()}.`);
        }else if( this.assignValue && !this.assignValue.type().check( value ) ){
            this.throwError(`"${this.key.raw()}" not match of type ${this.assignValue.type().toString()} to assignment type ${value.type().toString()}.`);
        }

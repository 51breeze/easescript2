...
class Property extends Stack{
    ...
    definition(){
        ...
        }else if(this.parentStack.isObjectExpression && !this.hasAssignmentPattern && this.init && this.init.isIdentifier ){
            const stack = this.scope.define( this.init.value() );
            return stack && stack.definition();
        }
        return null;
    }

    ...
    emit(syntax)
    {
        if( this.parentNode.type ==="ObjectPattern" ){
            ...
            if( is ){
                ...
                if( targetDesc ){
                    if( Utils.isStackByName(targetDesc,"Declarator") ){
                        const type = targetDesc.type(); 
                        if( !type.is( this.compilation.getType("Object") ) ){
                            this.throwErrorLine(`"${name}" is not object.`);
                        }
                    }else if( Utils.isStackByName(targetDesc,"CallExpression") ){
                        const refer = targetDesc.reference();

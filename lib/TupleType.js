...
class TupleType extends Type
{
    ...
    check( stack , flag=false ){
        ...
        if( !flag && stacks.length < this.requireCount ){
            return false;
        }
        ...
        return stacks.every( (item,index)=>{
            ...
            if( (!flag && baseType) && baseType !== restType ){
                return baseType.check( item , flag);
            }else{
                if( restType ){
                    return restType.check( item , flag);
                }else{
                    return this.elements.some( (host)=>{
                        return host.check( item , flag);
                    });
                }
            }
        });

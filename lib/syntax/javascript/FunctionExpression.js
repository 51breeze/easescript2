    ...
    emit(syntax){
        ...
        if( this.stack.async ){
            ...
            const expression = [
                ...
                before.join("\r\n"),
                `${startIndent}\t\treturn global.__awaiter(this, void 0, void 0, function (){`,
                insertBefore.join("\r\n"),
                `${startIndent}\t\t\treturn global.__generator(this, function (${hand}) {`
            ];
            const end = `${endIndent}\t\t});\r\n${endIndent}\t});\r\n${endIndent}}`;
        ...
        if( this.stack.isArrowFunction && this.stack.isExpression ){
            ...
            let content = before.concat(insertBefore.splice(0),body).join("\r\n");
            if( this.stack.isConstructor ){
                const event={properties:null};
                this.stack.parentStack.dispatcher("fetchClassProperty",event);
                if( event.properties ){
                    content += '\r\n'+this.semicolon(`\tObject.defineProperty(this,private,{value:${event.properties}})`);
                }
            }
            if( key ){

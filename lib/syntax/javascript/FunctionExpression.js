...
class FunctionExpression extends Syntax{
   emit(syntax){
       ...
       if( this.stack.isArrowFunction && this.stack.isExpression ){
           const content = before.concat(this.stack.insertBefore.splice(0), this.semicolon(`\treturn ${body}`) ).join("\r\n");
           return `${startIndent}function(${params.join(",")}){\r\n${content}\r\n${endIndent}}`;
       }else{
           const content = before.concat(this.stack.insertBefore.splice(0),body).join("\r\n");
           if( key ){

...
class PackageDeclaration extends Syntax{
    ...
    makeClass(syntax){
            ...
            for( var name in target ){
                ...
                if( item.isPropertyDefinition && !item.static ){
                    ...
                    if( modifier !=="private" ){
                       ...
                       content.push(`\tObject.defineProperty(${proto}, "${name}",{\r\n\tget:${getter},\r\n\tset:${setter}});`);
                    }
                    ...
                    content.push(`\tObject.defineProperty(${proto}, "${name}",{${accessor.join(",")}});`);
                }else{
                    const value = make(item,name,modifier);
                    content.push(`\tObject.defineProperty(${proto}, "${name}", {value:${value}});`);
                }
            }
        }
        ...
        const code = [
            ...
            refs.join("\r\n"),
            `\t${construct}`,
            `\tObject.defineProperty(${module.id},"prototype",{value:Object.create(${parentClass}.prototype)});`,
            `\tObject.defineProperty(${module.id}.prototype,"constructor",{value:${module.id}});`,
            content.join("\r\n"),

...
class Grammar {
    ...
    arrowFunctionExpression(node,scope,parent)
    {
        ...
        if( has ){
           ...
           fnScope.pushBefore( this.makeDeclarationVariable("const", this.makeAssign(name,"this",null,this.description.id) ) );
        }
    ...
    functionExpression(node, scope, parent,isArrow)
    {
        ...
        let body = node.body.body.map( (item)=>{
           return this.parser(item,scope,node);
        });

        body = scope.before.concat( body, scope.after )
                            .filter( val=>!!val )
                            .map( (expre)=>{
                                return this.semicolon( expre );
                            });

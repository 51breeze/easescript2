...
class Grammar {
    ...
    MemberExpression(node,scope,parent )
    {
        ...
        const getType=(name, base)=>{
            ...
            if( this.compilation.module.namespace )
            {
                base = this.compilation.getType(name, base);
                if( !base )
                {
                    this.compilation.throwError(`"${member.join(".")}" is not defined`, node.start )
                }
            }
            return base;
        }
    ...
    ObjectExpression(node,scope, parent)
    {
        ...
        this.setExpressionReference( new Type("Object", map) );
    ...
    {
        if( types.length == 1 ){
            this.expressionReference.push( !(types[0] instanceof Type) ? new Type( types[0] ) : types[0]);
        }else if(types.length > 1){
            types = types.map( item=> !(item instanceof Type) ? new Type( item ) : item );
    ...
    parserFunctionParams(scope,items,parent)
    {
        ...
        items.forEach( (item)=>{
            ...
            if( item.type === "AssignmentPattern" )
            {
                ...
                declar = new Parameter(name, defaultValue, type, "var");
    ...
    FunctionExpression(node,scope,parent,isArrow,isExpression)
    {
        ...
        this.setExpressionReference( new Type("Function",scope) );
    ...
    ArrayExpression(node,scope, parent){
        ...
        const elements = node.elements.map( (item)=>{
            let value = this.parser(item,scope,node);
        ...
        this.setExpressionReference( new Type("Array",map) );
    ...
    AssignmentExpression(node, scope, parent)
    {
        ...
        const value = this.parser( node.value, scope, node );
        const declar = scope.define(name);
        this.setAssignment(declar, this.getExpressionReference() );
        return this.makeAssignmentExpression(scope,name,value);
    }

    ThisExpression(node, scope, parent)
    {
        this.setExpressionReference( new Type( this.module.id ) );
    ...
    NewExpression(node, scope, parent)
    {
        ...
        this.setExpressionReference( new Type(callee) );
    ...
    ExpressionStatement(node,scope, parent)
    {
        ...
        //console.log( this.getExpressionReference() );
        //this.setExpressionReference( type );
        return this.semicolon( scope, expre );
    }
    ...
    VariableDeclaration(node,scope,parent)
    {
        ...
        node.declarations.forEach( (item)=>{
            ...
            declars.forEach( (declar)=>{
                ...
                {
                    declarations.push( this.makeAssign(declar.name,null,null,declar.acceptType) ); 
                }else{
                    declarations.push( this.makeAssign(declar.name,declar.initValue,declar.defaultValue,declar.acceptType) );  
                }
            });
        });
    ...
    VariableDeclarator(node,scope,parent)
    {
        if( node.id && node.id.type ==="ArrayPattern" )
        {
            ...
            if( initType==="ArrayExpression" )
            {
                initVlaue=node.init.elements.map( (item)=>{
                   return this.parser(item,scope,node.id);
                });

            }else{
                initVlaue = this.parser(node.init,scope,node.id);
            }
            ...
            return node.id.elements.map( (item,index)=>{
                ...
                if( initType==="ArrayExpression" ){
                    declar.initValue = this.makeComputeValue( initVlaue[ index ], init );  
                }else{
                    declar.initValue = this.makeComputeValue( `${initVlaue}[${index}]`, init);
                } 
                return declar;
        ...
        }else if( node.id && node.id.type ==="ObjectPattern" )
        {
            ...
            return node.id.properties.map( (item,index)=>{
                ...
                declar.defaultValue = init;
                if( initType==="ObjectExpression" ){
                    declar.initValue = this.makeComputeValue(initVlaue[id],init);
                }else{
                    declar.initValue = this.makeComputeValue(`${initVlaue}["${id}"]`,init);
                }
                return declar;
    ...
    EnumDeclaration(node,scope,parent)
    {
        ...
        node.value.expressions.forEach( (item,index)=>{
            ...
            map[ key ] = this.getExpressionReference() || new Type("Number");
    ...
    PropertyDefinition(node,scope, parent)
    {
        ...
        this.compilation.module.addMember(declar.name, declar);
    }

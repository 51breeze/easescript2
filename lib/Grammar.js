...
class Grammar {
    ...
    FunctionExpression(node){
        ...
        let rest = '';
        const params = node.arguments.map( item=>{
            if( item.isRest ){
    ...
    IfStatement(node,scope,parent)
    {
         ...
         const alter = '';
         if( alter ){
            alter `else{ ${alternate} }`;
         }
         return `if(${condition}){${consequent.join("\r\n")}}${alter}`;
    ...
    build(  )
    {
        console.log( this.compilation.stack.children[0] )
        // if( node )
        // {

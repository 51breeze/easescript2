...
class Grammar {
    ...
    ObjectExpression(node, scope, parent)
    {
        ...
        node.properties.forEach( item=>{
            ...
            const value = item.isSpreadElement ? this.build( item ) : this.build( item.value );
    ...
    {
        const elements =  [];
        let isSpreadElement= false;
        node.elements.forEach( item=>{
            const expre = this.build(item);
            if( item.isSpreadElement ){
    ...
    CallExpression(node)
    {
        const member = node.callee.member ? node.callee.member.map( item=>item.name ).join(".") : node.callee.value;
        const args   = node.arguments.map( item=>this.build(item) );
        const expre  = `${member}(${args.join(",")})`;
        return expre;
    }

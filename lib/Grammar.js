...
class Grammar {
    ...
    ObjectExpression(node, scope, parent)
    {
        ...
        return new Expression(node, properties, "Object", map);
    }
    ...
    ArrayExpression(node,scope, parent)
    {
        ...
        return new Expression(node,elements,"Array",elements);
    }
    ...
    ThisExpression(node, scope, parent)
    {
        ...
        const thisName = fnScope.isArrow ? fnScope.getThisRef() : "this";
        const description = scope.define( thisName );
        if( !description )
        {
            this.error( `"${thisName}" is not defined.`, node);
        }
        return {node,thisName,description};
    }
    ...
    WhenStatement(node,scope, parent)
    {
        const test = ( callback )=>{
            ...
            switch( name ){
                case "Runtime" :
                    consequent = callback( args );
                break;
            }
            return consequent;
        }

        const consequent = new BlockScope(scope);
        let alternate  = null;
        node.consequent.body.forEach( (item)=>{
            item = this.parser(item,consequent,node.consequent);
            if( item )
            {
                consequent.add( item );
            }
        ...
        if( node.alternate )
        {
            alternate = new BlockScope(scope);
            node.alternate.body.forEach( (item)=>{
                item = this.parser(item,alternate,node.alternate);
                if( item )
                {
                    alternate.add( item );
                }
    ...
    VariableDeclarator(node,scope,parent)
    {
        if( node.id && node.id.type ==="ArrayPattern" )
        {
            ...
            return node.id.elements.map( (item,index)=>{
                ...
                const declar = new Declarator(item, id, parent.kind, null, acceptType, init);
                scope.define(id, declar);
                if( initType==="ArrayExpression" ){
                    declar.initValue =  initVlaue.value[ index ] ? initVlaue.value[ index ].value : null;
                }else{
        ...
        }else if( node.id && node.id.type ==="ObjectPattern" )
        {
            ...
            return node.id.properties.map( (item,index)=>{
                ...
                if( initType==="ObjectExpression" ){
                    ...
                    declar.initValue = `${initValue.value}["${id}"]`;
                }
    ...
    ClassDeclaration(node, scope, parent)
    {
        ...
        const parseStruct = (flag)=>{
            return node.body.body.map( (item)=>{
                ...
                {
                   item = this.parser(item,classScope,node,flag);
                   if( flag && item){
                      body.push( item );
                   }
                }
    ...
    InterfaceDeclaration(node, scope, parent)
    {
        ...
        node.body.body.map( (item)=>{
            item = this.parser(item,scope,node,true);
            if( item ){
                body.push( item );
            }
        });

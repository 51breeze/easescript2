...
class Grammar {
    ...
    parserFunctionParams(scope,items,parent)
    {
        ...
        items.forEach( (item)=>{
            ...
            if( item.type === "AssignmentPattern" )
            {
                ...
                this.setAssignment(declar,scope,item.right,item);
    ...
    AssignmentExpression(node, scope, parent)
    {
        ...
        this.setAssignment(declar,scope,node.value,node);
    ...
    ReturnStatement(node,scope, parent)
    {
        ...
        const fnScope = scope.getScopeByType("function");
        const target = {};
        this.setAssignment(target,fnScope,node.argument,node);
        fnScope.returnItems.push( target.assign );
    ...
    VariableDeclarator(node,scope,parent)
    {
        if( node.id && node.id.type ==="ArrayPattern" )
        {
            ...
            return node.id.elements.map( (item,index)=>{
                ...
                if( item.type === "AssignmentPattern"  )
                {
                    init =  this.parser(item.right, scope, item)
                }else{
                    init = initType==="ArrayExpression" ? initVlaue[ index ] : `${initVlaue}[${index}]`;
                }
        ...
        const declar = new Declarator( id, init, acceptType, parent.kind);
        scope.define(id, declar);
        this.setAssignment(declar,scope,node.init,node);
        return [declar];
    }

    setAssignment(declar, scope, node, parent )
    {
        if( node )
        {
            if( node.type === "ArrowFunctionExpression" || node.type === "FunctionExpression")
            {
                declar.assign = scope.getChildByAt();
            ...
            }else if(  node.type === "Literal" )
            {
                if( node.regex ){
                    ...
                    declar.assign = node.value == node.raw ? "number" : "string";
                }

            }else if( node.type === "NewExpression" )
            {
                declar.assign = this.parser(node.callee,scope,parent);
            }
        }
    }
    ...
    parser(node,scope,parent=null)
    {
        ...
        try{
          return this[ node.type ](node,scope,parent);
           ...
           console.log( node.type , e.message, e )
        }
    }
}

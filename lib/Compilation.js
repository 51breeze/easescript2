...
class Compilation extends events.EventEmitter 
{
    ...
    parserFunctionParams(scope,items,parent)
    {
        ...
        items.forEach( (item)=>{
            ...
            if( item.type === "AssignmentPattern" )
            {
                ...
                this.setAssignment(declar, defaultValue, true);
    ...
    CallExpression(node, scope, parent)
    {
        ...
        return {node,parent,scope,callee,arguments:args,description};
    }
    ...
    VariableDeclarator(node,scope,parent)
    {
        ...
        }else if( node.id && node.id.type ==="ObjectPattern" )
        {
            ...
            return node.id.properties.map( (item,index)=>{
                ...
                if( initType ==="ObjectExpression" )
                {
                    ...
                    this.setAssignment(declar, declar.initValue );
        ...
        if( init )
        {
            this.setAssignment(declar, init);
        }
        declar.parent = parent;
        scope.define(id, declar);
        return declar;
    ...
    checkTupleType( typeElements , value)
    {
        if( value.description instanceof Declarator )
        {
            return this.checkTupleType(typeElements, value.description.initValue);
        } 

        if( value.elements && value.elements.length ===0 && value.description.id == "Array")
        {
            return true;
        }

        let result = null;
        for(var index=0; index < typeElements.length; index++)
        {
            result = this.checkAcceptType(typeElements[index], value);
            if( result === true ){
                return true;
            }
        }
    ...
    checkAcceptType(acceptType, value)
    {
        if( value.description instanceof Declarator )
        {
            return this.checkAcceptType(acceptType, value.description.initValue);
        } 

        if( acceptType.isArrayElement )
        {
            if( value.elements && value.description.id == "Array")
            {
                const typeElements= acceptType.typeElements;
                for( var index=0; index < value.elements.length;index++)
                {
                    const item = value.elements[ index ];
                    if( typeElements )
                    {
                        const isTuple = !(typeElements.length > index);
                        const result = !isTuple ? this.checkAcceptType(typeElements[index], item) : this.checkTupleType(typeElements,item);
                        if( result !== true )
                        {
                            if( isTuple )
                            {
                                return {result:false,left:typeElements,right:item};
                            }
                            return result;
                        }

                    }else if( !this.checkType(acceptType.description, item.description) )
                    {
                        return {result:false, left:acceptType, right:item};
                    }
                }

            }else
            {
                return {result:false, left:acceptType, right:value};
            }

        }else if( !this.checkType(acceptType.description, value.description) )
        {
            return {result:false, left:acceptType, right:value};
        }
        return true;
    ...
    setAssignment(declar, value , isParam = false )
    {
        let writeable = declar.kind==="var" || declar.kind==="let" || (declar.kind==="const" && !declar.assignment);
        if( writeable )
        {
            if( !(isParam && value.description.id ==="Nullable") )
            {
               const result = declar.acceptType ? this.checkAcceptType(declar.acceptType,value) : true;
               if( result !== true )
               {
                   if( declar.acceptType.isArrayElement ){
                       const types = (result.left instanceof Array ? result.left : [result.left]).map( item=>item.value );
                       this.error(`"${declar.key}" assignment array not match of element type. give a ${result.right.description.id}, can only be ${types.join(" | ")}.`,declar.node, declar.key);
                   }else{
                       this.error(`"${declar.key}" assignment value does not match of type. must be ${result.left.value}`,declar.node, declar.key);
                   }
               }
            }
    ...
    TypeDefinition(node,scope, parent)
    {
       ...
       type.isArrayElement = !!(typeElements || node.isArrayElement);
       type.typeElements   = typeElements;
       type.parent=parent;

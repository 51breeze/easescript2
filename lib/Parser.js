...
class SyntaxParser extends Parser {
    ...
    parseTypeStatement()
    {
        ...
        if( (this.type === tokTypes.name || this.type  === tokTypes._void) && this.type !== tokTypes.eof ){
            ...
            const parseProperty = ()=>{
                ...
                return this.finishNode(prop, "TypeObjectProperty")
            }
            ...
            return this.finishNode(node,"TypeObjectExpression");
        }
        else
        {
            this.unexpected();
    ...
    parseClassElement( constructorAllowsSuper )
    {
        ...
        const isConstruct = !isProperty && element && ( 
                    (element.key && element.key.name.toLowerCase()==="constructor") || 
        ...
        if( this.type === tokTypes.relational && this.value && this.value.charCodeAt(0) === 60 ){
            ...
            if( /\>\(/.test(str) && /\)\;?$/.test(str) ){
                ...
                while( this.type !== tokTypes.relational ){
                    genericity.push( this.parseTypeStatement() );

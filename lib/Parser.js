...
class SyntaxParser extends Parser {
    ...
    readToken( code ){
        ...
        if( this.type === tokTypes.relational && this.value === "<" && last === tokTypes.name ){
            this.leftHandRrelational = this.lastTokStart;
        }
        return token;
    }
        ...
        if( this.type === tokTypes.relational && this.value && this.value.charCodeAt(0) === 60 ){
            ...
            if( /\>\(/.test(str) && /\)\;?$/.test(str) ){
                ...
                while( this.type !== tokTypes.relational ){
                    generic.push( this.parseExprAtom() );

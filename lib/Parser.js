...
keywordTypes["as"] = new TokenType("as", {beforeExpr: true, binop: 7});
...
keywordTypes["override"] = new TokenType("override",{startsExpr: true});
...
class SyntaxParser extends Parser {
    ...
    readToken( code ){
        ...
        const last = this.__lastReadTokenType;
        const value = this.__lastReadTokenValue;
        const token = super.readToken(code);
        this.__lastReadTokenType = this.type;
        this.__lastReadTokenValue = this.value;
        if( this.type === tokTypes.relational &&  this.value === "<" ){
           // console.log( this.input.slice(this.lastTokStart,this.lastTokStart+20) )
        }
        return token;
    }
    ...
    parseClassId(node, isStatement)
    {
        ...
        node.id.genericType = this.parseGenericType();
        this.currentClassId = node.id;
    }

    parsePropertyName(prop){
        const node = super.parsePropertyName(prop);
        node.genericType = this.parseGenericType();
        return node;
    }
    ...
    parseExprAtom(refDestructuringErrors){
        ...
        if( tokTypes.relational === this.type ){
           // console.log( this.start,this.lastTokStart, this.lastTokEnd , this.lastTokStartLoc, this.lastTokEndLoc )
            //console.log( "===========",this.input.slice(this.lastTokStart, this.lastTokEnd )   )
           // console.log( "=====+++++======", this.input.slice(this.lastTokEnd, this.start).charCodeAt(0)   )
        }
        return node;
    }
    ...
    parseExprOp(left, leftStartPos, leftStartLoc, minPrec, noIn){
        ...
        const expre = super.parseExprOp(left, leftStartPos, leftStartLoc, minPrec, noIn);
        return expre;
    }

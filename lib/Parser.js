...
class SyntaxParser extends Parser {
    ...
    parseStatement(context, topLevel, exports, isStatic)
    {
        ...
        const genericType = this.parseGenericType();
        const expr = super.parseStatement(context, topLevel, exports);
        if( genericType ){
            expr.genericType = genericType;
        }
        return expr;
    }
    ...
    parseClassMethod(method, isGenerator, isAsync, allowsDirectSuper){
        ...
        method.genericType = this.parseGenericType();
    ...
    parseGenericType(){
        if( this.type === tokTypes.relational && this.value && this.value.charCodeAt(0) === 60 ) 
        {
            ...
            do{
                ...
                genericType.push( type );
                if( this.eat(tokTypes._extends) ){
                    if( tokTypes.name === this.type && this.value ==="keyof"){
                        const node = this.finishNode(this.startNode(), "Keyof"); 
